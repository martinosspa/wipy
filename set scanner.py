...
def main():
    ...
    for item in all_items:
        ...
        if riw.isInSet(item_url_name) and not item_url_name in scanned_items:
            items_in_sets = riw.request_item_info(item_url_name)
            total_buy_price = 0
            total_sell_price = 0

            for item_in_set in items_in_sets:
                set_name = riw.getSet(item)
                scanned_items.append(item_in_set)
            ...
            if total_buy_price < total_sell_price:
                print("{name} has {dif} plat profit".format(name=item_url_name, dif=total_sell_price-total_buy_price))
